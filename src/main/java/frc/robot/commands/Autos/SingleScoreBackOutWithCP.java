...
public class SingleScoreBackOutWithCP extends SequentialCommandGroup {
  /** Creates a new SingleScoreBackOutNonCableProtector. */
  StateHandler stateHandler = StateHandler.getInstance();
  public SingleScoreBackOutWithCP(SwerveSubsystem swerve) {

    final AutoFromPathPlanner strafeOutCP = new AutoFromPathPlanner(swerve, "StrafeOutCP", 2.5, 2, false, true,
    true);
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new InstantCommand(() -> swerve.resetOdometryForState(strafeOutCP.getInitialState())),
